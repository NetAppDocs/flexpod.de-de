...
image:flexpod-genomics-image8.png["Ein Diagramm, das einen Genomik-Workflow von der Probenentnahme über die Sequenzierung, Variantenanalyse, EHR-Integration und prädiktive Modelle darstellt."]
...
+
image:flexpod-genomics-image10.png["Eine Tabelle mit VLAN-IP-Reservierungen, die Bereiche, Subnetzmasken sowie Verwendungszwecke für Management, VM, NFS und iSCSI zeigt"]

. Konfigurieren Sie iSCSI-basierte Boot-LUNs auf der ONTAP SVM.
+
image:flexpod-genomics-image9.png["Ein Screenshot von ONTAP System Manager zeigt sechs ESXi Boot-LUNs in der Storage-VM Healthcare_SVM."]

. Zuordnen von LUNs zu iSCSI-Initiatorgruppen
+
image:flexpod-genomics-image11.png["Ein Screenshot, der zeigt, dass ESXi_Boot_Lun_1 der Initiatorgruppe GenomicsESXi_1 des iSCSI-Initiators zugeordnet ist."]

+
image:flexpod-genomics-image12.png["Ein Screenshot des ONTAP System Manager, der die Details der ESXi-Boot-LUN zeigt, die der Initiatorgruppe GenomicsESXi_2 zugeordnet ist"]

. Installation von vSphere 7.0 mit iSCSI Boot
. Registrieren Sie ESXi-Hosts mit vCenter.
+
image:flexpod-genomics-image13.png["Ein Screenshot des vSphere Client-Inventars, der den Genomics-Ordner mit ESXi Hosts und virtuellen Maschinen zeigt"]

. Bereitstellung eines NFS-Datenspeichers `infra_datastore_nfs` Auf dem ONTAP Storage.
+
image:flexpod-genomics-image14.png["Ein Screenshot von ONTAP System Manager, der das ausgewählte NFS-Volume Healthcare_Workload_NFS_Vol online anzeigt"]

. Fügen Sie den Datastore zum vCenter hinzu.
+
image:flexpod-genomics-image16.png["Ein Screenshot des vSphere Client zeigt die Zusammenfassung des NFS 3 Datenspeichers infra_datastore_nfs"]

. Fügen Sie mithilfe von vCenter einen NFS-Datenspeicher zu den ESXi Hosts hinzu.
+
image:flexpod-genomics-image15.png["Ein Screenshot des vSphere Client zeigt infra_datastore_nfs Hosts, die mit normalem Status verbunden sind"]
...
+
image:flexpod-genomics-image17.png["Ein Screenshot eines Linux-Terminals, der zeigt, dass /dev/sdb unter /mnt/genomics mit 2 % Auslastung eingehängt ist"]
