---
sidebar: sidebar 
permalink: hybrid-cloud/cloud-insights_use_cases_01.html 
keywords: use, cases, active, iq, integration, dashboards, troubleshooting, storage, optimization, custom, real, time 
summary: Mit Cloud Insights für das Monitoring Ihrer FlexPod Datacenter Lösung eingerichtet und konfiguriert, können wir einige der Aufgaben untersuchen, die Sie auf dem Dashboard durchführen können, um Ihre Umgebung zu bewerten und zu überwachen. In diesem Abschnitt werden fünf primäre Anwendungsfälle für Cloud Insights vorgestellt. 
---
= Anwendungsfälle
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
:linkattrs: 
:imagesdir: ../media/


[role="lead"]
Mit Cloud Insights für das Monitoring Ihrer FlexPod Datacenter Lösung eingerichtet und konfiguriert, können wir einige der Aufgaben untersuchen, die Sie auf dem Dashboard durchführen können, um Ihre Umgebung zu bewerten und zu überwachen. In diesem Abschnitt werden fünf primäre Anwendungsfälle für Cloud Insights vorgestellt:

* Digital Advisor Integration
* Über Echtzeit-Dashboards entdecken
* Erstellen benutzerdefinierter Dashboards
* Erweiterte Fehlerbehebung
* Storage-Optimierung




[[digital-advisor-integration]]
== Digital Advisor Integration

Cloud Insights ist vollständig in die Digital Advisor Storage-Monitoring-Plattform integriert. Ein ONTAP System, das als Teil einer FlexPod Datacenter-Lösung bereitgestellt wird, ist automatisch so konfiguriert, dass es über die in jedem System integrierte AutoSupport Funktion Informationen an NetApp zurücksendet. Diese Berichte werden nach einem Zeitplan oder dynamisch generiert, sobald ein Fehler im System erkannt wird. Die über AutoSupport übermittelten Daten werden aggregiert und in leicht zugänglichen Dashboards unter dem Menü Digital Advisor in Cloud Insights angezeigt.



[[access-digital-advisor-information-through-the-cloud-insights-dashboard]]
=== Zugriff auf Digital Advisor-Informationen über das Cloud Insights Dashboard

Um auf die Digital Advisor-Informationen über das Cloud Insights Dashboard zuzugreifen, sind die folgenden Schritte erforderlich:

. Klicken Sie auf die Option Data Collector im Menü Admin auf der linken Seite.
+
image:cloud-insights_image13.png["Ein Screenshot, der das Admin-Menü mit ausgewählten Data Collectors in Cloud Insights zeigt"]

. Filtern Sie nach dem bestimmten Data Collector in Ihrer Umgebung. In diesem Beispiel wurde der Begriff FlexPod nach dem Begriff gefiltert.
+
image:cloud-insights_image23.png["Ein Screenshot, der die Seite „Datensammler“ gefiltert nach FlexPod Datacenter zeigt."]

. Klicken Sie auf den Data Collector, um eine Übersicht über die Umgebung und die Geräte zu erhalten, die von diesem Collector überwacht werden.
+
image:cloud-insights_image24.png["Ein Screenshot der FlexPod Datacenter-Datensammler-Zusammenfassung mit Ereigniszeitleiste und gemeldetem Speichergerät"]

+
Klicken Sie unter der Geräteliste unten auf den Namen des überwachten ONTAP Storage-Systems. Auf diese Weise wird ein Dashboard mit Informationen angezeigt, die über das System erfasst wurden. Dazu gehören folgende Details:

+
** Modell
** Familie
** ONTAP-Version
** Bruttokapazität
** IOPS-Durchschnitt
** Durchschnittliche Latenz
** Durchschnittlicher Durchsatz
+
image:cloud-insights_image25.png["Ein Screenshot des Speicher-Dashboards aa14-a800 mit Zusammenfassung, Risiken, Benutzerdaten und Latenzdiagramm"]

+
Außerdem befindet sich auf dieser Seite im Abschnitt „Leistungsrichtlinien“ ein Link zum NetApp Digital Advisor.

+
image:cloud-insights_image26.png["Ein Screenshot, der Leistungsrichtlinien mit 35 von Active IQ erkannten Risiken und einem externen Link-Symbol zeigt"]



. Ein neuer Browser-Tab wird geöffnet und führt zur Seite für Risikominderung, auf der angezeigt wird, welche Knoten betroffen sind, wie kritisch die Risiken sind und welche geeignete Maßnahme zur Behebung der festgestellten Probleme erforderlich ist. Der Link für Digital Advisor stellt diese Funktion bereit.
+
image:cloud-insights_image27.png["Ein Screenshot von Active IQ Digital Advisor, der Gesundheitsrisiken und Korrekturmaßnahmen für ein Speichersystem auflistet"]





[[explore-real-time-dashboards]]
=== Dashboards in Echtzeit

Cloud Insights bietet Echtzeit-Dashboards mit Informationen, die von dem in einer FlexPod Datacenter-Lösung implementierten ONTAP Storage-System abgefragt wurden. Die Cloud Insights-Erfassungseinheit erfasst Daten in regelmäßigen Abständen und füllt das Standard-Storage-System-Dashboard mit den erfassten Informationen aus.



==== Zugriff auf Echtzeitdiagramme über das Cloud Insights Dashboard

Im Dashboard des Speichersystems wird angezeigt, wenn der Data Collector die Informationen zuletzt aktualisiert hat. Ein Beispiel hierfür ist in der Abbildung unten dargestellt.

image:cloud-insights_image28.png["Ein Screenshot der Cloud Insights Data Collector Details zeigt die erfolgreiche Datenerfassung vor 3 Minuten um 13:21 Uhr"]

Standardmäßig werden auf dem Storage-System-Dashboard mehrere interaktive Diagramme angezeigt, die systemweite Metriken vom zu beforschenden Storage-System oder von jedem einzelnen Node zeigen, darunter Latenz, IOPS und Durchsatz im Abschnitt Expert View. Beispiele für diese Standarddiagramme sind in der folgenden Abbildung dargestellt.

image:cloud-insights_image29.png["Ein Screenshot der Cloud Insights Expert View mit Diagrammen zu Latenz, IOPS und Durchsatz sowie Ressourcendetails"]

Standardmäßig werden in den Diagrammen Informationen der letzten drei Stunden angezeigt. Sie können diese jedoch in der Dropdown-Liste oben rechts im Dashboard des Storage-Systems auf eine Reihe verschiedener Werte oder einen benutzerdefinierten Wert festlegen. Dies ist in der Abbildung unten dargestellt.

image:cloud-insights_image30.png["Ein Screenshot des Storage Dashboards mit dem Dropdown-Menü für den Zeitbereich, das „Letzte 15 Minuten“ bis „Benutzerdefiniert“ anzeigt."]



[[create-custom-dashboards]]
=== Erstellen benutzerdefinierter Dashboards

Nutzen Sie nicht nur die Standard-Dashboards, die systemweite Informationen anzeigen, sondern erstellen Sie mithilfe von Cloud Insights vollständig angepasste Dashboards, mit denen Sie sich auf die Ressourcenauslastung für bestimmte Storage-Volumes in der FlexPod Datacenter Lösung konzentrieren können. Daher werden die in der konvergenten Infrastruktur implementierten Applikationen, die von diesen Volumes für eine effektive Ausführung abhängen. Auf diese Weise lässt sich eine bessere Visualisierung bestimmter Applikationen und der in der Datacenter-Umgebung genutzten Ressourcen erzielen.



==== Erstellen Sie ein angepasstes Dashboard zur Bewertung von Storage-Ressourcen

Gehen Sie wie folgt vor, um ein angepasstes Dashboard zur Bewertung von Storage-Ressourcen zu erstellen:

. Wenn Sie ein angepasstes Dashboard erstellen möchten, bewegen Sie den Mauszeiger über Dashboards im Hauptmenü von Cloud Insights, und klicken Sie in der Dropdown-Liste auf + Neues Dashboard.
+
image:cloud-insights_image31.png["Ein Screenshot des Cloud Insights Dashboards-Menüs mit hervorgehobenem „+ New Dashboard“"]

+
Das Fenster Neues Dashboard wird geöffnet.

. Benennen Sie das Dashboard, und wählen Sie den Typ des Widgets aus, mit dem die Daten angezeigt werden. Sie können aus einer Reihe von Diagrammtypen oder sogar Notizen oder Tabellentypen auswählen, um die erfassten Daten anzuzeigen.
+
image:cloud-insights_image32.png["Ein Screenshot der neuen Dashboard-Widget-Auswahlkacheln, einschließlich Diagrammen, Messinstrumenten, Notizen und Tabellen"]

. Wählen Sie im Menü Variable hinzufügen benutzerdefinierte Variablen aus.
+
Dadurch können die präsentierten Daten fokussiert werden, um spezifische oder speziellere Faktoren anzuzeigen.

+
image:cloud-insights_image33.png["Ein Screenshot, der das Menü „Variable hinzufügen“ mit einem Suchfeld und einer Variablenliste auf einem neuen Dashboard zeigt"]

. Wenn Sie ein benutzerdefiniertes Dashboard erstellen möchten, wählen Sie den Widget-Typ aus, den Sie verwenden möchten, beispielsweise ein Kreisdiagramm zur Anzeige der Storage-Auslastung nach Volume:
+
.. Wählen Sie das Widget „TIE-Diagramm“ aus der Dropdown-Liste „Widget hinzufügen“ aus.
.. Benennen Sie das Widget mit einer beschreibenden Kennung, z. B. `Capacity Used`.
.. Wählen Sie das anzuzeigende Objekt aus. Sie können beispielsweise nach dem Schlüsselwort Volume suchen und auswählen `volume.performance.capacity.used`.
.. Um nach Storage-Systemen zu filtern, verwenden Sie den Filter, und geben Sie den Namen des Storage-Systems in der FlexPod Datacenter Lösung ein.
.. Passen Sie die angezeigten Informationen an. Standardmäßig werden bei dieser Auswahl ONTAP-Daten-Volumes angezeigt und die Top 10 aufgelistet.
.. Um das benutzerdefinierte Dashboard zu speichern, klicken Sie auf Speichern.
+
image:cloud-insights_image34.png["Ein Screenshot, der ein für die Speichervolumenkapazität genutztes Tortendiagramm-Widget zeigt."]

+
Nach dem Speichern des benutzerdefinierten Widgets kehrt der Browser zur Seite Neues Dashboard zurück, auf der das neu erstellte Widget angezeigt wird, und ermöglicht die Durchführung interaktiver Aktionen, wie z. B. das Ändern des Datenabfragungsperiode.

+
image:cloud-insights_image35.png["Ein Screenshot, der ein Cloud Insights Dashboard mit einem Tortendiagramm-Widget zur genutzten Kapazität zeigt"]







[[advanced-troubleshooting]]
=== Erweiterte Fehlerbehebung

Cloud Insights ermöglicht die Anwendung fortschrittlicher Methoden zur Fehlerbehebung in jeder Speicherumgebung einer FlexPod Datacenter konvergenten Infrastruktur. Durch die Nutzung der Komponenten jeder der oben genannten Funktionen: Digital Advisor Integration, Standard-Dashboards mit Echtzeit Statistiken und benutzerdefinierte Dashboards, werden potenzielle Probleme frühzeitig erkannt und schnell behoben. Anhand der Risikoliste in Digital Advisor können Kunden gemeldete Konfigurationsfehler finden, die zu Problemen führen könnten, sowie gemeldete Bugs und gepatchte Versionen von Code, die diese beheben können. Das Beobachten der Echtzeit-Dashboards auf der Cloud Insights Startseite kann dabei helfen, Muster in der Systemleistung zu entdecken, die ein früher Indikator für ein aufkommendes Problem sein könnten, und dessen schnelle Behebung unterstützen. Die Möglichkeit, benutzerdefinierte Dashboards zu erstellen, ermöglicht es Kunden, sich auf die wichtigsten Assets ihrer Infrastruktur zu konzentrieren und diese direkt zu überwachen, um sicherzustellen, dass die Ziele zur Geschäftskontinuität erreicht werden können.



[[storage-optimization]]
=== Storage-Optimierung

Neben der Fehlerbehebung ist es möglich, die von Cloud Insights gesammelten Daten zur Optimierung des ONTAP Storage-Systems, das in einer FlexPod Datacenter konvergenten Infrastruktur-Lösung eingesetzt wird, zu nutzen. Weist ein Volume eine hohe Latenz auf, beispielsweise weil mehrere VMs mit hohen Leistungsanforderungen denselben Datenspeicher nutzen, werden diese Informationen im Cloud Insights Dashboard angezeigt. Mit diesen Informationen kann ein Speicheradministrator entscheiden, eine oder mehrere VMs entweder auf andere Volumes zu migrieren, Speichervolumes zwischen Aggregat-Tiers oder zwischen Knoten im ONTAP Storage-System zu verschieben, was zu einer leistungsoptimierten Umgebung führt. Die aus der Integration von Digital Advisor mit Cloud Insights gewonnenen Informationen können Konfigurationsprobleme aufzeigen, die zu einer geringeren als erwarteten Leistung führen, und die empfohlene Korrekturmaßnahme bereitstellen, die, wenn sie umgesetzt wird, etwaige Probleme beheben und ein optimal abgestimmtes Storage-System sicherstellen kann.
